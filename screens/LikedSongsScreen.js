...
const LikedSongsScreen = () => {
  ...
  const playNextTrack = async () => {
    if (currentSound) {
      await currentSound.stopAsync();
      await currentSound.unloadAsync(); // Ensure the current sound is unloaded before playing a new one
      setCurrentSound(null);
    }
    currentIndex.current += 1;
    if (currentIndex.current < savedTracks.length) {
      const nextTrack = savedTracks[currentIndex.current];
      setCurrentTrack(nextTrack);
      extractColors();
      await play(nextTrack);
    } else {
      console.log("end of playlist");
      currentIndex.current = savedTracks.length - 1; // Reset to the last track index
    }
  };
  ...
  const playPreviousTrack = async () => {
    ...
    if (currentIndex.current >= 0) {
      const prevTrack = savedTracks[currentIndex.current];
      setCurrentTrack(prevTrack);
      extractColors();
      await play(prevTrack);
    } else {
      console.log("start of playlist");
      currentIndex.current = 0; // Reset to the first track index
    }
  };
